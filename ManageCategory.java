...
public class ManageCategory extends JFrame implements ActionListener {
    ...
    ManageCategory() {
        ...
        l1.setBounds(50, 55, 100, 30);
        l2.setBounds(50, 105, 100, 30);
        l3.setBounds(500, 20, 150, 30);

        t1.setBounds(150, 55, 170, 30);
        t2.setBounds(150, 105, 170, 30);

        b1.setBounds(50, 230, 90, 30);
        b2.setBounds(160, 230, 90, 30);
        b3.setBounds(270, 230, 90, 30);
    ...
    private void fetchAndPopulateTable() {
        try {
            ...
            Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "SYSDBA", "sys2708");
        ...
        if (e.getSource() == b1) {
            ...
            try {
                ...
                Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "SYSDBA",
                        "sYS2708");
                String sql = "INSERT INTO CATEGORY VALUES(" + categoryID + ",'" + name + "')";
                Statement st = con.createStatement();
            ...
            try {
                ...
                // If category ID is found, update the fields
                if (rowIndex != -1) {
                    ...
                    Class.forName("oracle.jdbc.driver.OracleDriver");
                    Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "SYSDBA", "sys2708");
                    String sql = "UPDATE CATEGORY SET CNAME = '" + name + "'  WHERE CATID = " + categoryID;
                    Statement st = con.createStatement();
                    int i = st.executeUpdate(sql);
                    if (i > 0) {
